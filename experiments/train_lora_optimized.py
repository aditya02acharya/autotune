"""
Optimized LoRA Training for Prompt Injection Detection

Implements the recommended stack from research:
- LoRA-GA initialization (gradient-approximated)
- DoRA (weight decomposition)
- LoRA+ (16x higher LR for B matrices)
- All 7 linear layer types (not just Q/V)
"""
...
class OptimizedLoRATrainer:
    """Trainer with optimized LoRA configuration."""

    # All 7 linear layer types for Qwen2.5
    ALL_LINEAR_LAYERS = ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]

    def __init__(self, model_name="Qwen/Qwen2.5-0.5B-Instruct", output_dir="data/adapters/optimized"):
    ...
    def setup_model(self, use_4bit=True, use_dora=True, rank=16, target_all_layers=True):
        ...
        # Optimized LoRA config with DoRA
        print(f"Applying optimized LoRA adapters...")
    ...
    def train_epoch(self, dataloader, optimizer, scheduler, epoch, use_lora_plus=True, lora_plus_lr_ratio=16):
        """Train for one epoch with LoRA+ optimization."""
        self.model.train()
    ...
    def train(self, train_data, val_data, epochs=3, batch_size=8, lr=2e-4, use_lora_plus=True):
        """Full training loop with optimized settings."""
        print(f"\nTraining with optimized configuration:")
        print(f"  - Epochs: {epochs}")
        print(f"  - Batch size: {batch_size}")
        print(f"  - Base LR: {lr}")
        print(f"  - LoRA+: {use_lora_plus}")
        ...
        # LoRA+ optimizer: different LRs for A and B matrices
        # B matrices get 16x higher learning rate
        if use_lora_plus:
            print("  - LoRA+ LR for B matrices: 16x base LR")
...
def main():
    print("="*70)
    print("OPTIMIZED LoRA Training - Prompt Injection Detection")
    print("Features: DoRA + All Linear Layers + LoRA+ LR")
    print("="*70)
    ...
    print("\n2. Creating optimized trainer...")
    trainer = OptimizedLoRATrainer(
        model_name="Qwen/Qwen2.5-0.5B-Instruct",
        output_dir="data/adapters/optimized"
    )
    ...
    print("\n3. Training with optimized configuration...")
    best_metrics = trainer.train(
        train_data=train_data,
        val_data=val_data,
        epochs=3,
        batch_size=8,
        lr=2e-4,  # Recommended base LR
        use_lora_plus=True,
    )
    ...
    with open("data/adapters/optimized/results.json", "w") as f:
        json.dump(results, f, indent=2)

    # Log to experiment progress
    with open("data/experiment_progress.tsv", "a") as f:
        f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')}\toptimized_lora\tcompleted\t{json.dumps(results)}\n")
